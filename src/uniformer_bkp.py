...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        dpr = [0 for x in torch.linspace(0, drop_path_rate, sum(depth))]  # stochastic depth decay rule
...
@register_model
def uniformer_small_original(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 1, 1],
        embed_dim=[24, 24, 24, 24], head_dim=24, mlp_ratio=1, qkv_bias=True,
        norm_layer=nn.Identity, **kwargs)
