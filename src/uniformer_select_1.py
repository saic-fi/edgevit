...
class SparseFFController(nn.Module):
    def __init__(self, in_features, d_lowrank=None, out_features=None, act_layer=nn.GELU, drop=0., keepdim=12):
        super().__init__()
    ...
    def forward(self, x):
        ...
        B, N, d_ffn = pred_score.shape
        # x = self.drop(x)
        pred_score = pred_score.view(B, N, self.keep_dim, -1)
        if self.training:
            Flag = True if torch.rand(1) > 0.5 else False
        else:
            Flag = True
        mask = F.gumbel_softmax(pred_score, hard=Flag, dim=-1)
        print(mask)
        mask = mask.reshape(B, N, d_ffn)
        return mask
...
def easy_gather(x, indices):
    # x: B,N,C; indices: B,N
    B, N, num_head, C = x.shape
    N_new = indices.shape[1]
    offset = torch.arange(B, dtype=torch.long, device=x.device).view(B, 1) * N
    indices = indices + offset
    out = x.reshape(B * N, num_head, C)[indices.view(-1)].reshape(B, N_new, num_head, C)
    return out
...
class Attention_(nn.Module):
    ...
    def token_select(self, q: torch.Tensor, k: torch.Tensor, v:torch.Tensor, N:int, avg_cls_attn: torch.Tensor):
        cls_token = q[:, :, 0:1, ...]
        cls_attn = (cls_token @ k.permute(0, 2, 1, 3).transpose(-2, -1)) * self.scale
        cls_attn = cls_attn.softmax(dim=-1)
        cls_attn = cls_attn[:, :, 0, :].sum(1)  # (B, 1, N)
        cls_attn = self.trade_off * cls_attn + (1. - self.trade_off) * avg_cls_attn
        N_ = int(N // self.sr_ratio)
        _, indices = torch.sort(cls_attn, dim=1, descending=True)
        indices = indices[:, :N_]
        k_ = easy_gather(k, indices)
        v_ = easy_gather(v, indices)
        return k_.permute(0, 2, 1, 3), v_.permute(0, 2, 1, 3), cls_attn

    def forward(self, x:torch.Tensor, H:int, W:int, avg_cls_attn: torch.Tensor):
        B, N, C = x.shape
        cls_token = x.mean(dim=1, keepdim=True)
        x_w_cls_token = torch.cat([cls_token, x], dim=1)
        q = self.q(x_w_cls_token).reshape(B, N + 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        kv = self.kv(x).reshape(B, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 1, 3, 4)
        k, v = kv[0], kv[1]

        if self.sr_ratio > 1:
            k, v, avg_cls_attn = self.token_select(q, k, v, N, avg_cls_attn)
        else:
            k, v = k.permute(0, 2, 1, 3), v.permute(0, 2, 1, 3)

        q_ = q[:, :, 1:, :]  # remove cls_token
        q_ = self.sampler(q_)
        ...
        if self.sr_ratio > 1:
            x = x.permute(0, 2, 1).reshape(B, C, int(H/self.sr_ratio), int(W/self.sr_ratio))
            x = self.upsample(x)
            x = x.reshape(B, C, -1).permute(0, 2, 1)

        return x, avg_cls_attn


class SABlock_(nn.Module):
    ...
    def forward(self, x, cls_attn):
        ...
        # else:
        x_, cls_attn =self.attn(self.norm1(x), H, W, cls_attn)
...
class CSABlock_(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1.):
        super().__init__()
        self.cblock = CBlock(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                             drop_path, act_layer, norm_layer)
        self.sablock = SABlock_(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                               drop_path, act_layer, norm_layer, sr_ratio)

    def forward(self, x, cls_attn):
        x = self.cblock(x)
        x, cls_attn = self.sablock(x, cls_attn)
        return x, cls_attn
...
class Attention(nn.Module):
    ...
    def token_select(self, q: torch.Tensor, k: torch.Tensor, v:torch.Tensor, N:int):
        cls_token = q[:, :, 0:1, ...]
        cls_attn = (cls_token @ k.permute(0, 2, 1, 3).transpose(-2, -1)) * self.scale
        cls_attn = cls_attn.softmax(dim=-1)
        cls_attn = cls_attn[:, :, 0, :].sum(1)  # (B, 1, N)
    ...
    def forward(self, x:torch.Tensor, H:int, W:int):
        ...
        if self.sr_ratio > 1:
            k, v, avg_cls_attn = self.token_select(q, k, v, N)
            q = q[:, :, 1:, :]  # remove cls_token
        else:
            k, v = k.permute(0, 2, 1, 3), v.permute(0, 2, 1, 3)
            avg_cls_attn = None
...
class SABlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #     x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        # else:
        x_, cls_attn = self.attn(self.norm1(x), H, W)
        x = x + self.drop_path(x_)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        x = x.transpose(1, 2).reshape(B, N, H, W)
...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.blocks3_1 = nn.ModuleList([
            CSABlock(
                 dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
                 drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[0 + depth[0] + depth[1]], norm_layer=norm_layer,
                 sr_ratio=sr_ratios[2])])

        self.blocks3_2 = nn.ModuleList([
           CSABlock_(
                dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks3_1:
            x, cls_attn = blk(x)
        for blk in self.blocks3_2:
            x, cls_attn = blk(x, cls_attn)
...
@register_model
def uniformer_small_twins_select_1(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[2, 1, 2, 2],
