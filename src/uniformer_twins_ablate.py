...
class Attention_ConvSR(nn.Module):
    """
...
class Attention_MaxSR(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, linear=False):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
...
class CSABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.sablock = SABlock(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                               drop_path, act_layer, norm_layer, sr_ratio, SA)
...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=None, conv_stem=False, sr_ratios=[8,4,2,1], SA=Attention):
        """
        Args:
        ...
        self.blocks4 = nn.ModuleList([
            SABlock(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]+depth[2]], norm_layer=norm_layer, sr_ratio=sr_ratios[3], SA=SA)
...
class PVT(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
                 ...
                 sr_ratios=[8, 4, 2, 1], SA=Attention):
        """
        Args:
            depth (list): depth of each stage
            img_size (int, tuple): input image size
        ...
        self.blocks1 = nn.ModuleList([
            SABlock(
                ...
                sr_ratio=sr_ratios[0], SA=SA)
            for i in range(depth[0])])
        self.blocks2 = nn.ModuleList([
            SABlock(
                dim=embed_dim[1], num_heads=num_heads[1], mlp_ratio=mlp_ratio[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i + depth[0]], norm_layer=norm_layer,
                sr_ratio=sr_ratios[1], SA=SA)
            for i in range(depth[1])])
        self.blocks3 = nn.ModuleList([
            SABlock(
                dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i + depth[0] + depth[1]], norm_layer=norm_layer,
                sr_ratio=sr_ratios[2], SA=SA)
            for i in range(depth[2])])
        self.blocks4 = nn.ModuleList([
            SABlock(
                dim=embed_dim[3], num_heads=num_heads[3], mlp_ratio=mlp_ratio[3], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i + depth[0] + depth[1] + depth[2]],
                norm_layer=norm_layer, sr_ratio=sr_ratios[3], SA=SA)
            for i in range(depth[3])])
        self.norm = nn.BatchNorm2d(embed_dim[-1])
...
@register_model
def uniformer_twins_05G_ConvSR(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 3, 2],
        embed_dim=[36, 72, 144, 288], head_dim=36, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[8,4,2,1], SA=Attention_ConvSR, **kwargs)
    model.default_cfg = _cfg()
    return model

@register_model
def uniformer_twins_05G_AvgSR(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 3, 2],
        embed_dim=[36, 72, 144, 288], head_dim=36, mlp_ratio=[4]*4, qkv_bias=True,
...
@register_model
def uniformer_twins_05G_MaxSR(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 3, 2],
        embed_dim=[36, 72, 144, 288], head_dim=36, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[8,4,2,1], SA=Attention_MaxSR, **kwargs)
    model.default_cfg = _cfg()
    return model

@register_model
def uniformer_pvt_05G_ConvSR(pretrained=True, **kwargs):
    model = PVT(
        depth=[2, 2, 5, 2],
        embed_dim=[36, 72, 144, 288], head_dim=36, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[8,4,2,1], SA=Attention_ConvSR, **kwargs)
