...
@register_model
def uniformer_twins_05G(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 3, 2],
...
@register_model
def uniformer_twins_1G(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 3, 1],
...
@register_model
def uniformer_twins_2G(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 2, 5, 3],
        embed_dim=[48, 96, 240, 384], head_dim=48, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4,2,2,1], **kwargs)
...
@register_model
def uniformer_small_twins_plus(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 5, 2],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
