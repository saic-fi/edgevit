...
class CBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, split_ratio=1.):
        super().__init__()
        self.pos_embed = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)

        dim = int(dim * split_ratio)
    ...
    def forward(self, x):
        ...
        if self.split_ratio != 1.0:
            x1, x2 = torch.split(x, [int(N * self.split_ratio), int(N * (1 - self.split_ratio))], dim=1)
            x1 = x1 + self.drop_path(self.conv2(self.attn(self.conv1(self.norm1(x1)))))
...
class SABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1., split_ratio=1.0):
        super().__init__()
        self.pos_embed = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
...
class CSABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1., split_ratio=1.):
        super().__init__()
        self.cblock = CBlock(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                             drop_path, act_layer, norm_layer, split_ratio)
        self.sablock = SABlock(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                               drop_path, act_layer, norm_layer, sr_ratio, split_ratio)
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.blocks1 = nn.ModuleList([
            CSABlock(
                ...
                sr_ratio=sr_ratios[0], split_ratio=split_ratio[0])
        ...
        self.blocks2 = nn.ModuleList([
            CSABlock(
                ...
                sr_ratio=sr_ratios[1], split_ratio=split_ratio[1])
        ...
        self.blocks3 = nn.ModuleList([
            CSABlock(
                ...
                sr_ratio=sr_ratios[2], split_ratio=split_ratio[2])
        ...
        self.blocks4 = nn.ModuleList([
            SABlock(
                ...
                sr_ratio=sr_ratios[3], split_ratio=split_ratio[3])
...
@register_model
def uniformer_small_twins_plus_deconv_shuffle(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 4, 2],
        embed_dim=[64*2, 128*2, 256*2, 512*2], head_dim=64, mlp_ratio=[1] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1],
        split_ratio=[0.5, 0.5, 0.5, 0.5], **kwargs)
    model.default_cfg = _cfg()
    return model

# @register_model
# def uniformer_small_twins_plus_channel_split_plus(pretrained=True, **kwargs):
#     model = UniFormer(
#         depth=[1, 3, 5, 3],
#         embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=[4]*4, qkv_bias=True,
#         norm_layer=partial(nn.LayerNorm, eps=1e-6), split_ratio=[0.5, 0.5, 0.5, 1.], **kwargs)
#     model.default_cfg = _cfg()
#     return model
