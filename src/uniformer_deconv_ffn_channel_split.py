...
class SABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(int(dim*split_ratio))
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=int(dim*split_ratio), hidden_features=int(mlp_hidden_dim*split_ratio), act_layer=act_layer, drop=drop)
        global layer_scale
        self.ls = layer_scale
        if self.ls:
            global init_value
...
@register_model
def uniformer_small_twins_plus_deconv_ffn_channel_split(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 5, 2],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1],
        split_ratio=[0.5, 0.5, 0.5, 1.], **kwargs)
    model.default_cfg = _cfg()
    return model
