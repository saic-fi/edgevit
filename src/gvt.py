...
class GroupBlock(TimmBlock):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super(GroupBlock, self).__init__(dim, num_heads)
        del self.attn
        if ws == 1:
            # print('GSA')
...
class CPVTV2(PyramidVisionTransformer):
    ...
    def forward_features(self, x):
        ...
        # for i in range(len(self.depths)):
        for i, (patch_embed, pos_drop, block, pos_block) in enumerate(zip(self.patch_embeds, self.pos_drops, self.blocks, self.pos_block)):
            x, (H, W) = patch_embed(x)
            x = pos_drop(x)
            for j, blk in enumerate(block):
                x = blk(x, H, W)
                if j == 0:
                    x = pos_block(x, H, W)  # PEG here
            if i < len(self.depths) - 1:
                x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
...
@register_model
def alt_gvt_small_2G(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 6, 3], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
        **kwargs)
    model.default_cfg = _cfg()
    # print(model)
    return model
...
@register_model
def alt_gvt_small_05G(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4,
        depths=[2, 2, 4, 2],
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
        **kwargs)
    model.default_cfg = _cfg()
    return model

@register_model
def alt_gvt_small_1G(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4,
        depths=[1, 2, 4, 2],
