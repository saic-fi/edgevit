...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., keep_ratio=1):
        super().__init__()
...
#     def __init__(self, in_features, d_lowrank=None, out_features=None, act_layer=nn.GELU, drop=0., keep_ratio=0.5):
#         super().__init__()
#         out_features = out_features or in_features
#         self.keep_dim = in_features // 4
#         self.T = 0.1
#         self.fc1 = nn.Linear(in_features, d_lowrank)
#         # self.act = act_layer()
#         self.fc2 = nn.Linear(d_lowrank, out_features)
#         self.act1 = nn.Sigmoid()
#         self.act2 = LWTA(keep_ratio)
#         # self.drop = nn.Dropout(drop)
#
#     def forward(self, x):
#         if len(x.shape) == 4:
...
#         B, d_ffn = pred_score.shape
#         pred_score = pred_score.view(B, self.keep_dim, -1)
#         pred_score = self.act1(pred_score)
#         mask = self.act2(pred_score)
#         if reshape:
#             mask = mask.reshape(B, d_ffn, 1, 1)
#         else:
#             mask = mask.reshape(B, 1, d_ffn)
#         return mask
...
class SparseMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
        ...
        self.sparse_controller = SparseFFController(in_features, in_features // 4, hidden_features,
                                                    keep_ratio=keep_ratio)

    def forward(self, x):
        mask = self.sparse_controller(x)
        x = self.fc1(x)
        x = x * mask
        x = self.act(x)
...
class SparseMlpInference(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
                 keep_ratio=0.5):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = nn.Parameter(torch.ones(in_features, hidden_features), requires_grad=True) #nn.Linear(in_features, hidden_features)
        self.act = act_layer()

        self.fc2 = nn.Parameter(torch.ones(hidden_features, in_features),  requires_grad=True)  #nn.Linear(hidden_features, out_features)
        self.drop = nn.Dropout(drop)

        self.sparse_controller = SparseFFController(in_features, in_features // 4, hidden_features,
                                                    keep_ratio=keep_ratio)

        self.keep_dim = int( keep_ratio * hidden_features)
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape    # Assume B=1, N, C
        mask = self.sparse_controller(x).squeeze(0)

        indices = torch.nonzero(mask)
        # indices = indices[:, -1].reshape(N, self.keep_dim)
        # fc1_ = torch.stack([self.fc1[:, indices[i]] for i in range(len(indices))], dim=0)
        # fc2_ = torch.stack([self.fc2[indices[i], :] for i in range(len(indices))], dim=0)

        indices = indices[:, -1].flatten()
        fc1_ = self.fc1[:, indices].reshape(C, N, self.keep_dim).transpose(0,1)
        fc2_ = self.fc2[indices, :].reshape(N, C, self.keep_dim).transpose(1,2)

        tmp = torch.bmm(x.transpose(1, 0), fc1_)
        # tmp= torch.einsum('bnc,cnh->bnh', x, fc1_)

        tmp = self.act(tmp)
        x = torch.bmm(tmp, fc2_).transpose(1, 0)
        # x = torch.einsum('bnh, nch->bc', tmp, fc2_)
        # x = self.fc1(x)
        # x = x * mask
        # x = self.act(x)
        #
...
class CBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, keep_ratio=0.5):
        super().__init__()
...
class SABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1., keep_ratio=0.5):
        super().__init__()
        self.pos_embed = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
...
class CSABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.cblock = CBlock(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                             drop_path, act_layer, norm_layer, keep_ratio)
        self.sablock = SABlock(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                               drop_path, act_layer, norm_layer, sr_ratio, keep_ratio)
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.blocks4 = nn.ModuleList([
            SABlock(
                ...
                sr_ratio=sr_ratios[3], keep_ratio=keep_ratio
            )
...
@register_model
def uniformer_small_twins(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 4, 2],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=[4] * 4, qkv_bias=True,
...
@register_model
# def uniformer_small_twins_plus_upsample(pretrained=True, **kwargs):
#     model = UniFormer(
#         depth=[1, 1, 5, 2],
#         embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=[4]*4, qkv_bias=True,
#         norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
@register_model
def uniformer_small_twins_plus_deconv(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 5, 2],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=[4] * 4, qkv_bias=True,
...
@register_model
def uniformer_x_small_twins_plus_deconv(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 3, 1],
        embed_dim=[36, 72, 144, 288], head_dim=18, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], **kwargs)
    model.default_cfg = _cfg()
    return model
...
# @register_model
# def uniformer_small_twins_plus_deconv_2G(pretrained=True, **kwargs):
#     model = UniFormer(
#         depth=[1, 1, 5, 2],
#         embed_dim=[48, 96, 240, 384], head_dim=24, mlp_ratio=[4] * 4, qkv_bias=True,
#         norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 1, 1], **kwargs)
#     model.default_cfg = _cfg()
#     return model


@register_model
def uniformer_small_twins_plus_deconv_1G_sparse(pretrained=True, **kwargs):
    model = UniFormer_(
        depth=[2, 1, 2, 2],
        embed_dim=[48, 96, 240, 384], head_dim=24, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], keep_ratio=0.25, **kwargs)
    model.default_cfg = _cfg()
    return model


@register_model
def uniformer_small_twins_plus_deconv_1G_sparse_elementwise_50(pretrained=True, **kwargs):
    model = UniFormer_(
        depth=[2, 1, 2, 2],
        embed_dim=[48, 96, 240, 384], head_dim=24, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], keep_ratio=0.5, **kwargs)
    model.default_cfg = _cfg()
    return model


@register_model
def uniformer_small_twins_plus_deconv_1G_sparse_elementwise_75(pretrained=True, **kwargs):
    model = UniFormer_(
        depth=[2, 1, 2, 2],
        embed_dim=[48, 96, 240, 384], head_dim=24, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], keep_ratio=0.75, **kwargs)
    model.default_cfg = _cfg()
    return model


@register_model
def uniformer_tiny_twins_plus_deconv(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 3, 1],
        embed_dim=[48, 96, 192, 384], head_dim=24, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], **kwargs)
    model.default_cfg = _cfg()
    return model
...
@register_model
def uniformer_tiny_twins_plus_deconv_1(pretrained=True, **kwargs):
    model = UniFormer_(
        depth=[2, 1, 3, 1],
        embed_dim=[48, 96, 192, 384], head_dim=24, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], **kwargs)
...
@register_model
def uniformer_small_twins_plus_deconv_plus(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 2, 5, 3],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2, 1], **kwargs)
...
@register_model
def uniformer_small_twins_plus_sr(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 5, 2],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=[4] * 4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
