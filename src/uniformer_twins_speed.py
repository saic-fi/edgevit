...
class LSABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, ws=1.):
        super().__init__()
        self.pos_embed = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
        self.norm1 = norm_layer(dim)
...
class CSABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.cblock = LSABlock(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                             drop_path, act_layer, norm_layer, ws)
...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=None, conv_stem=False, sr_ratios=[8,4,2,1], ws=[7,7,7,7]):
        """
        Args:
        ...
        self.blocks1 = nn.ModuleList([
            CSABlock(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, sr_ratio=sr_ratios[0], ws=ws[0])
            for i in range(depth[0])])
        self.blocks2 = nn.ModuleList([
            CSABlock(
                dim=embed_dim[1], num_heads=num_heads[1], mlp_ratio=mlp_ratio[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]], norm_layer=norm_layer, sr_ratio=sr_ratios[1], ws=ws[1])
            for i in range(depth[1])])
        self.blocks3 = nn.ModuleList([
            CSABlock(
                dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]], norm_layer=norm_layer, sr_ratio=sr_ratios[2], ws=ws[2])
...
@register_model
def uniformer_twins_deconv_05G_lsa(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 3, 2],
...
@register_model
def uniformer_twins_deconv_1G_lsa(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 3, 1],
...
@register_model
def uniformer_twins_deconv_2G(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 2, 5, 3],
        embed_dim=[48, 96, 240, 384], head_dim=48, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4,2,2,1], **kwargs)
...
@register_model
def uniformer_xx_small_twins_deconv(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[2, 1, 3, 1],
        embed_dim=[48, 96, 240, 384], head_dim=48, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4,2,2,1], **kwargs)
...
@register_model
def uniformer_small_twins_plus_deconv_1G(pretrained=True, **kwargs):
    model = UniFormer_(
        depth=[2, 1, 2, 2],
...
@register_model
def uniformer_x_small_twins_deconv(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 5, 2],
        embed_dim=[48, 96, 240, 384], head_dim=48, mlp_ratio=[4]*4, qkv_bias=True,
...
@register_model
def uniformer_small_twins_plus_deconv_test(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 1, 1],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4,2,2,1], **kwargs)
...
def uniformer_small_twins_plus_deconv_2G(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[1, 1, 4, 3],
        embed_dim=[64, 128, 256, 512], head_dim=64, mlp_ratio=[4,4,4,4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4, 2, 2,1], **kwargs)
    model.default_cfg = _cfg()
    return model
